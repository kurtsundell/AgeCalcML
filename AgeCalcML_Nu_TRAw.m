%% AGECALCML_NU_TRAW MATLAB code for AgeCalcML_Nu_TRAw.fig %%
function varargout = AgeCalcML_Nu_TRAw(varargin)
gui_Singleton = 1;
gui_State = struct('gui_Name',mfilename,'gui_Singleton',gui_Singleton,'gui_OpeningFcn', @AgeCalcML_Nu_TRAw_OpeningFcn,'gui_OutputFcn',@AgeCalcML_Nu_TRAw_OutputFcn,'gui_LayoutFcn',[],'gui_Callback',[]);
...
function AgeCalcML_Nu_TRAw_OpeningFcn(hObject, eventdata, H, varargin)
H.output = hObject;
set(H.WM_STD2,'Visible','off')
guidata(hObject, H);
function varargout = AgeCalcML_Nu_TRAw_OutputFcn(hObject, eventdata, H) 
...
%{
if get(H.reject_yes,'Value') == 0
	current_status_num_STD1 = ones(length(sigx_sq_STD1),1);
end
...
if get(H.reject_yes,'Value') == 1
	...
	for i = 1:data_count
		if STD1_idx(i,1) == 1
			current_status_num_STD1(count,1) = current_status_num(i,1);
