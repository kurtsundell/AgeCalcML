%% AGECALCML_NU_TRA_01 MATLAB code for AgeCalcML_Nu_TRA_01.fig %%
function varargout = AgeCalcML_Nu_TRA_01(varargin)
gui_Singleton = 1;
gui_State = struct('gui_Name',mfilename,'gui_Singleton',gui_Singleton,'gui_OpeningFcn', @AgeCalcML_Nu_TRA_01_OpeningFcn,'gui_OutputFcn',@AgeCalcML_Nu_TRA_01_OutputFcn,'gui_LayoutFcn',[],'gui_Callback',[]);
...
function AgeCalcML_Nu_TRA_01_OpeningFcn(hObject, eventdata, H, varargin)
H.output = hObject;
set(H.WM_STD2,'Visible','off')
guidata(hObject, H);
function varargout = AgeCalcML_Nu_TRA_01_OutputFcn(hObject, eventdata, H)
...
for i = 1:data_count
	...
	elseif STD2_idx(i,1) == 1 || STD3_idx(i,1) == 1
		SAMPLE_CONCORDIA(i+1,:) = [num2cell(ratio75(i,:)), num2cell(ratio75_err(i,:)), num2cell(ratio68(i,:)), num2cell(err68m(i,:)), num2cell(rho(i,:)), ...
			Age68(i,:), Age68_err(i,:), Age67(i,:), Age67_err(i,:), Best_Age(i,:), Best_Age_err(i,:), Age82(i,:), Age82_err(i,:)];
	end
end
...
if sum(STD2_idx) > 1
	...
	age_label3_x = STD2_68*(1/STD2_67)*137.818;
...
if get(H.tertiary,'Value') == 1
	WM_Data_hi = 558 + 558*str2num(get(H.percdev,'String'))*.01;
	WM_Data_lo = 558 - 558*str2num(get(H.percdev,'String'))*.01;
end

if get(H.tertiary,'Value') == 2
	WM_Data_hi = 1099 + 1099*str2num(get(H.percdev,'String'))*.01;
	WM_Data_lo = 1099 - 1099*str2num(get(H.percdev,'String'))*.01;
end
...
if sum(STD3_idx) > 1
	...
	age_label3_x = STD3_68*(1/STD3_67)*137.818;
...
s = 1/sqrt(sum(1./(data2(:,2).*data2(:,2)))); % SE
s_abs = s/100*tt;
MSWD_STD3 = 1/(length(data2(:,1))-1).*sum(((data2(:,1)- (sum(data2(:,1)./(data2(:,2).^2))/sum(1./(data2(:,2).^2))) ).^2)./((data2(:,2)./2).^2)); %MSWD at 1 sigma matches Isoplot


tt4 = sprintf('%.2f ', tt);
ss4 = sprintf('%.2f ', s);
mswd4 = sprintf('%.2f ', MSWD_STD3);
