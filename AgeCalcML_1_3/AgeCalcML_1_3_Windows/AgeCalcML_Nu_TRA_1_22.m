%% AGECALCML_NU_TRA_1_22 MATLAB code for AgeCalcML_Nu_TRA_1_22.fig %%
function varargout = AgeCalcML_Nu_TRA_1_22(varargin)
gui_Singleton = 1;
gui_State = struct('gui_Name',mfilename,'gui_Singleton',gui_Singleton,'gui_OpeningFcn', @AgeCalcML_Nu_TRA_1_22_OpeningFcn,'gui_OutputFcn',@AgeCalcML_Nu_TRA_1_22_OutputFcn,'gui_LayoutFcn',[],'gui_Callback',[]);
...
function AgeCalcML_Nu_TRA_1_22_OpeningFcn(hObject, eventdata, H, varargin)
H.output = hObject;
set(H.WM_STD2,'Visible','off')
guidata(hObject, H);
function varargout = AgeCalcML_Nu_TRA_1_22_OutputFcn(hObject, eventdata, H) 
...
elseif get(H.method,'Value') == 2 % 300/hour
		...
		end_idx = t0_idx + 46; 
	if IC == 1
		start_idx_IC = t0_idx - 14;
		end_idx_IC = t0_idx + 45;
	end 
	samp_length = 60;
	for i = 1:data_count
		values_all(1:samp_length,1:cols,i) = values_tmp(start_idx(i,1):end_idx(i,1),1:cols);
	...
	if IC == 1
		for i = 1:data_count
			...
			baseline(1:12,4:7,i) = values_all(1:12,4:7,i);
			integration(1:35,4:7,i) = values_all(16:50,4:7,i);
		end
	end	
...
%{
if get(H.MSWD_FORCED,'Value') == 1
	data = [BLS_68_corr, BLS_68_err];
	FC = 'std';
	FC_idx = strfind(sample(:,1), FC);
	...
	t = sum(WM_data(:,1)./(WM_data(:,2).*WM_data(:,2))) / sum(1./(WM_data(:,2).*WM_data(:,2))); % Weighted Mean
	data2 = WM_data;
	data2(:,2) = data2(:,2).*2; % double the uncertainty to get the MSWD at 1 sigma.... 
	s = 1/sqrt(sum(1./(data2(:,2).*data2(:,2)))); % SE
	s_abs = s/100*t;
	MSWD68 = 1/(length(data2(:,1))-1).*sum(((data2(:,1)- (sum(data2(:,1)./(data2(:,2).^2))/sum(1./(data2(:,2).^2))) ).^2)./((data2(:,2)./2).^2)); %MSWD at 1 sigma matches Isoplot
	if MSWD68 > 1
		add_to = [0.001:0.001:10]'; % percent
		for i = 1:10000
			data_adj = WM_data; % absolute
			data_adj(:,2) = data_adj(:,2)./data_adj(:,1).*100; % convert to percent
			data_adj(:,2) = data_adj(:,2) + add_to(i,1); % percent
			data_adj(:,2) = data_adj(:,2).*data_adj(:,1).*.01; %convert to absolute
			data_adj2(:,1) = data_adj(:,1);
			data_adj2(:,2) = data_adj(:,2).*2; % doubled absolute
			s_adj = 1/sqrt(sum(1./(data_adj2(:,2).*data_adj2(:,2))));
			MSWD_adj(i,1) = 1/(length(data_adj2(:,1))-1).*sum(((data_adj2(:,1)- (sum(data_adj2(:,1)./(data_adj2(:,2).^2))/sum(1./(data_adj2(:,2).^2))) ).^2)./((data_adj2(:,2)./2).^2));
			clear data_adj
		end
		[val, idx] = find(abs(MSWD_adj(:,1)-1) < 0.01 & MSWD_adj > 1);
		dispersion_1sig_added68 = add_to(val(1,1),1); % percent
		BLS_68_err = BLS_68_err + dispersion_1sig_added68;
	end
	if MSWD68 < 1
		add_to = -1.*[0.001:0.001:10]'; % percent
		for i = 1:10000
			data_adj = WM_data; % absolute
			data_adj(:,2) = data_adj(:,2)./data_adj(:,1).*100; % convert to percent
			data_adj(:,2) = data_adj(:,2) + add_to(i,1); % percent
	...
	s = 1/sqrt(sum(1./(data2(:,2).*data2(:,2)))); % SE
	s_abs = s/100*t;
	MSWD68f = 1/(length(data2(:,1))-1).*sum(((data2(:,1)- (sum(data2(:,1)./(data2(:,2).^2))/sum(1./(data2(:,2).^2))) ).^2)./((data2(:,2)./2).^2)); %MSWD at 1 sigma matches Isoplot

	data = [BLS_67_corr, BLS_67_err];
	FC = 'std';
	FC_idx = strfind(sample(:,1), FC);
	FC_idx = abs(cellfun(@isempty,FC_idx)-1);
	for i = 1:length(data(:,1))
	...
	s = 1/sqrt(sum(1./(data2(:,2).*data2(:,2)))); % SE
	s_abs = s/100*t;
	MSWD67 = 1/(length(data2(:,1))-1).*sum(((data2(:,1)- (sum(data2(:,1)./(data2(:,2).^2))/sum(1./(data2(:,2).^2))) ).^2)./((data2(:,2)./2).^2)); %MSWD at 1 sigma matches Isoplot
	if MSWD67 > 1
		add_to = [0.001:0.001:10]'; % percent
		for i = 1:10000
			data_adj = WM_data; % absolute
		...
		[val, idx] = find(abs(MSWD_adj(:,1)-1) < 0.01 & MSWD_adj > 1);
		dispersion_1sig_added67 = add_to(val(1,1),1); % percent
		BLS_67_err = BLS_67_err + dispersion_1sig_added67;
	end
	if MSWD67 < 1
		add_to = -1.*[0.001:0.001:10]'; % percent
		for i = 1:10000
	...
	s = 1/sqrt(sum(1./(data2(:,2).*data2(:,2)))); % SE
	s_abs = s/100*t;
	MSWD67f = 1/(length(data2(:,1))-1).*sum(((data2(:,1)- (sum(data2(:,1)./(data2(:,2).^2))/sum(1./(data2(:,2).^2))) ).^2)./((data2(:,2)./2).^2)); %MSWD at 1 sigma matches Isoplot
set(H.FCMSWD,'String',strcat(sprintf('%.2f ', MSWD68), {',  '}, sprintf('%.2f ', MSWD67)))
set(H.FCcorr,'String',strcat(sprintf('%.2f ', dispersion_1sig_added68), {'%,  '}, sprintf('%.2f ', dispersion_1sig_added67), {'%'}))
set(H.FCMSWDf,'String',strcat(sprintf('%.2f ', MSWD68f), {', '}, sprintf('%.2f ', MSWD67f)))
end
%}
