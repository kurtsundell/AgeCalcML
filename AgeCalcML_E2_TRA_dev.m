%% AGECALCML_E2_TRA_DEV MATLAB code for AgeCalcML_E2_TRA_dev.fig %%

%% SET DEFAULT COMMAND LINE AND HANDLE STRUCTURE %%
function varargout = AgeCalcML_E2_TRA_dev(varargin)
gui_Singleton = 1;
gui_State = struct('gui_Name',mfilename,'gui_Singleton',gui_Singleton,'gui_OpeningFcn',@AgeCalcML_E2_TRA_dev_OpeningFcn,'gui_OutputFcn',@AgeCalcML_E2_TRA_dev_OutputFcn,'gui_LayoutFcn',[],'gui_Callback',[]);
...
function AgeCalcML_E2_TRA_dev_OpeningFcn(hObject, eventdata, H, varargin)
H.output = hObject;
guidata(hObject, H);

function varargout = AgeCalcML_E2_TRA_dev_OutputFcn(hObject, eventdata, H)
...
set(H.tSL,'Visible','off')
set(H.tFC,'Visible','off')
set(H.sliderSL,'Visible','off')
set(H.sliderFC,'Visible','off')
...
if TREE == 1
	...
	perc_FC = get(H.calibslider,'Value');
	perc_SL = 1 - get(H.calibslider,'Value');
	set(H.sliderSL,'String',round(perc_SL*100,1))
	set(H.sliderFC,'String',round(perc_FC*100,1))
	...
	SL_idx = strfind(sample, SL);
	R33_idx = strfind(sample, R33);
	NIST_idx = strfind(sample, NIST);
	
	FC_idx = abs(cellfun(@isempty,FC_idx)-1);
	SL_idx = abs(cellfun(@isempty,SL_idx)-1);
	R33_idx = abs(cellfun(@isempty,R33_idx)-1);
	NIST_idx = abs(cellfun(@isempty,NIST_idx)-1);
	sample_idx = abs((FC_idx + SL_idx + R33_idx + NIST_idx) - 1);
	
	Scan = numbers(:,1);
	Time = numbers(:,2) - numbers(1,2);
	ACFm = numbers(:,3)./64;
	
	ACFavg = movmean(ACFm,500);
	...
	for i = 1:length(numbers(:,1))-1
		if M029(i,1) < 500000 && M029(i+1,1) > 500000 && M029(i-1,1) < 500000 && M029(i+2,1) > 500000 && M029(i-2,1) < 500000 && M029(i+3,1) > 500000
			t0(i,1) = 1;
			t0idx(i,1) = i;
		else
			t0(i,1) = 0;
	...
	t0idx_start = t0idx(t0idx>0,1);
	%t0idx_end = t0idx(t0idx>0,1) + 45;
	...
	T_start = t0idx_start - 10;
	T_end = t0idx_start + 53;
	
	for i = 1:data_count
		values_all(1:64,1:8,i) = [M202(T_start(i,1):T_end(i,1)), M204(T_start(i,1):T_end(i,1)), M206(T_start(i,1):T_end(i,1)), M207(T_start(i,1):T_end(i,1)), ...
			M208(T_start(i,1):T_end(i,1)), M232(T_start(i,1):T_end(i,1)), M235(T_start(i,1):T_end(i,1)), M238(T_start(i,1):T_end(i,1))];
	end
	
	T_start2 = t0idx_start - 9;
	T_end2 = t0idx_start + 40;	
	
	for i = 1:data_count
		M_Parsed(1:50,:,i) = M_All(T_start2(i,1):T_end2(i,1),:);
	end	
	...
	for i = 1:42
		for j = 1:32
			for k = 1:data_count
				M_BS(i,j,k) = M_Parsed(i+8,j,k) - mean(M_Parsed(1:7,j,k));
			end
		end
	end
	...
	for j = 1:32
		for k = 1:data_count
			M_BS(43,j,k) = abs(mean(M_BS(3:22,j,k)));
		end
	end
	...
	if get(H.stoichSi,'Value') == 1 % yes normalize to stoichiometric Si
		for j = 1:32
			for k = 1:data_count
				if NIST_idx(k,1) == 1
					M_BS(44,j,k) = M_BS(43,j,k)/(M_BS(43,2,k)/16069);
				else
					M_BS(44,j,k) = M_BS(43,j,k)/(M_BS(43,2,k)/7178.6);
				end
			end
		end
	end
	...
	if get(H.stoichSi,'Value') == 0 % no do not normalize to stoichiometric Si, but rather average Si from 91500
		for m = 1:data_count
			if SL_idx(m,1) == 1
				SiSL(m,1) = M_BS(43,2,k);
			else
				SiSL(m,1) = 0;
			end
		end
		SiSLm = mean(nonzeros(SiSL));
		for j = 1:32
			for k = 1:data_count
				if NIST_idx(k,1) == 1
					M_BS(44,j,k) = M_BS(43,j,k)/(M_BS(43,2,k)/16069);
				else
					M_BS(44,j,k) = M_BS(43,j,k)/(M_BS(43,2,k)*(SiSLm/M_BS(43,2,k))/7178.6);
	...
	for i = 1:data_count
		if SL_idx(i,1) == 1
			STD_SL(i,1:23) = M_BS(44,1:23,i);
			STD_SL(i,24) = M_BS(44,30,i);
			STD_SL(i,25) = M_BS(44,32,i);
		end
	end
	STD_SL( all(~STD_SL,2), : ) = [];
	STD_SLm = mean(STD_SL,1);
	
	for i = 1:data_count
		if NIST_idx(i,1) == 1
			STD_NIST(i,1:23) = M_BS(44,1:23,i);
			STD_NIST(i,24) = M_BS(44,30,i);
			STD_NIST(i,25) = M_BS(44,32,i);
		end
	end
	STD_NIST( all(~STD_NIST,2), : ) = [];
	STD_NISTm = mean(STD_NIST,1);
	
	for i = 1:data_count
		if FC_idx(i,1) == 1
			STD_FC(i,1:23) = M_BS(44,1:23,i);
			STD_FC(i,24) = M_BS(44,30,i);
			STD_FC(i,25) = M_BS(44,32,i);
		end
	end
	...
	ALC_SL = [1.82	153230	15.84	3.36	3.89	132.6	1.74	0.005	2.55	0.05	0.52	0.69	0.13	2.65	1.02	10.83	4.21	19.33	4.33	49.33	8.40	9729.0	0.25	158.7	901.2];
	STD_SLc = ALC_SL.*scalar./STD_SLm;
	...
	STD_NISTc = Jochum.*scalar./STD_NISTm;
	...
	ALC_FC = [1.00	153230	232.50	3.54	18.94	1913.00	3.50	0.011	9.82	0.26	3.60	6.56	0.26	35.51	14.20	153.50	58.04	241.87	47.94	316.10	80.58	11140	1.54	197.9	363.8];
	STD_FCc = ALC_FC.*scalar./STD_FCm;
	
	for k = 1:data_count
		for j = 1:23
			ppm_Balance(k,j) = 0.0000001 + abs(perc_FC*M_BS(44,j,k)*STD_FCc(1,j) + perc_SL*M_BS(44,j,k)*STD_SLc(1,j));
		end
		ppm_Balance(k,24) = 0.0000001 + abs(perc_FC*M_BS(44,30,k)*STD_FCc(1,24) + perc_SL*M_BS(44,30,k)*STD_SLc(1,24));
		ppm_Balance(k,25) = 0.0000001 + abs(perc_FC*M_BS(44,32,k)*STD_FCc(1,25) + perc_SL*M_BS(44,32,k)*STD_SLc(1,25));
	end
	...
	for i = 1:data_count
		if SL_idx(i,1) == 1
			if ppm_Average(i,1) > str2num(get(H.filtAl,'String')) || ppm_Average(i,3) > str2num(get(H.filtP,'String'))
				SL_idx(i,1) = 0;
				sample(i,1) = {'SxxL'};
			end
		end
	...
	for k = 1:data_count
		for j = 1:25
			if  SL_idx(k,1) == 1
				STD_SLp(k,j) = 100*(ppm_Average(k,j) - ALC_SL(1,j)) / ALC_SL(1,j);
				ChonNormSL(k,j) = ChonNorm_MS95(k,j);
			end
			if  FC_idx(k,1) == 1
				STD_FCp(k,j) = 100*(ppm_Average(k,j) - ALC_FC(1,j)) / ALC_FC(1,j);
				ChonNormFC(k,j) = ChonNorm_MS95(k,j);
			end
			if  NIST_idx(k,1) == 1
				STD_NISTp(k,j) = 100*(ppm_Average(k,j) - Jochum(1,j)) / Jochum(1,j);
				ChonNormNIST(k,j) = ChonNorm_MS95(k,j);
			end
			if  sample_idx(k,1) == 1
	...
	STD_SLp( all(~STD_SLp,2), : ) = [];
	STD_SLpm = mean(STD_SLp,1);
	STD_SLps = std(STD_SLp,1);
	
	STD_FCp( all(~STD_FCp,2), : ) = [];
	STD_FCpm = mean(STD_FCp,1);
	STD_FCps = std(STD_FCp,1);
	
	STD_NISTp( all(~STD_NISTp,2), : ) = [];
	STD_NISTpm = mean(STD_NISTp,1);
	STD_NISTps = std(STD_NISTp,1);
	
	ChonNormSL( all(~ChonNormSL,2), : ) = [];
	ChonNormFC( all(~ChonNormFC,2), : ) = [];
	ChonNormNIST( all(~ChonNormNIST,2), : ) = [];
	
	ChonNormUnknownstmp( all(~ChonNormUnknownstmp,2), : ) = [];
	
	% for Tree norm plot
	
	ChonNormAcceptedSL = ALC_SL(1,8:21)./chon_norm(1,8:21);
	ChonNormAcceptedFC = ALC_FC(1,8:21)./chon_norm(1,8:21);
	ChonNormAcceptedNIST = Jochum(1,8:21)./chon_norm(1,8:21);
...
if TREE == 0
	...
	for i = 1:DataLength
		...
		elseif isnan(numbers(i,4)) == 1
			...
			M202ap(i,m) = numbers(i,4);
		end
		%end
	end
	
	M204ap = zeros(DataLength,1);
	for i = 1:DataLength
		%for m = 1:4
		if isnan(numbers(i,8)) == 1
			M204ap(i,m) = 10000000000;
		elseif isnan(numbers(i,7)) == 1
			M204ap(i,m) = numbers(i,8)*ACF(i,1);
		...
		elseif isnan(numbers(i,7)) == 1
			...
			M204ap(i,m) = numbers(i,7);
		end
		%end
	end
	
	M206ap = zeros(DataLength,1);
	for i = 1:DataLength
		%for m = 1:4
		if isnan(numbers(i,10)) == 1
			M206ap(i,m) = numbers(i,11).*ACF(i,1);
		elseif numbers(i,10) == 0 && numbers(i,11) > 2000
			M206ap(i,m) = numbers(i,11).*ACF(i,1);
		else
			M206ap(i,m) = numbers(i,10);
		end
		%end
	end
	
	M207ap = zeros(DataLength,1);
	for i = 1:DataLength
		%for m = 1:4
		if isnan(numbers(i,13)) == 1
			M207ap(i,m) = numbers(i,14).*ACF(i,1);
		elseif numbers(i,13) == 0 && numbers(i,14) > 2000
			M207ap(i,m) = numbers(i,14).*ACF(i,1);
		else
			M207ap(i,m) = numbers(i,13);
		end
		%end
	end
	
	M208ap = zeros(DataLength,1);
	for i = 1:DataLength
		%for m = 1:4
		if isnan(numbers(i,16)) == 1
			M208ap(i,m) = numbers(i,17).*ACF(i,1);
		elseif numbers(i,16) == 0 && numbers(i,17) > 2000
			M208ap(i,m) = numbers(i,17).*ACF(i,1);
		else
			M208ap(i,m) = numbers(i,16);
		end
		%end
	end
	
	M232ap = zeros(DataLength,1);
	for i = 1:DataLength
		%for m = 1:4
		if isnan(numbers(i,19)) == 1
			M232ap(i,m) = numbers(i,20).*ACF(i,1);
		elseif numbers(i,19) == 0 && numbers(i,20) > 2000
			M232ap(i,m) = numbers(i,20).*ACF(i,1);
		else
			M232ap(i,m) = numbers(i,19);
		end
		%end
	end
	
	M235ap = zeros(DataLength,1);
	for i = 1:DataLength
		%for m = 1:4
		if isnan(numbers(i,22)) == 1
			M235ap(i,m) = numbers(i,23).*ACF(i,1);
		elseif numbers(i,22) == 0 && numbers(i,23) > 2000
			M235ap(i,m) = numbers(i,23).*ACF(i,1);
		else
			M235ap(i,m) = numbers(i,22);
		end
		%end
	end
	
	M238ap = M235ap.*137.818;
	...
	for i = 1:data_count
		values_all(1:64,1:8,i) = [M202ap(T_start(i,1):T_end(i,1)), M204ap(T_start(i,1):T_end(i,1)), M206ap(T_start(i,1):T_end(i,1)), M207ap(T_start(i,1):T_end(i,1)), ...
			M208ap(T_start(i,1):T_end(i,1)), M232ap(T_start(i,1):T_end(i,1)), M235ap(T_start(i,1):T_end(i,1)), M238ap(T_start(i,1):T_end(i,1))];
	end	
...
UPBdata = zeros(46,15,data_count);
...
if TREE == 0
	for i = 1:data_count
		for j = 1:8
			if j == 3
				UPB_pre(i,j) = (sum(values_all(1:9,j,i))-max(values_all(1:9,j,i))-min(values_all(1:9,j,i)))/7;
			else
				UPB_pre(i,j) = (sum(values_all(3:9,j,i))-max(values_all(3:9,j,i))-min(values_all(3:9,j,i)))/5;
			end
		end
	end
end
...
for i = 1:data_count
	for j = 1:43
		if mean(values_all(j+10:j+13,1,i))-UPB_pre(i,1) < 1
			UPBdata(j,1,i) = 1;
		else
			UPBdata(j,1,i) = mean(values_all(j+10:j+13,1,i))-UPB_pre(i,1); %202
		end
	end
end
UPBdata(44:46,1,:) = [UPBdata(43,1,:);UPBdata(43,1,:);UPBdata(43,1,:)];

for i = 1:data_count
	for j = 1:43
		if mean(values_all(j+10:j+13,2,i)) - UPB_pre(i,2) - (UPBdata(j,1,i)/4.3) < 1
			UPBdata(j,2,i) = 1;
		else
			UPBdata(j,2,i) = mean(values_all(j+10:j+13,2,i)) - UPB_pre(i,2) - (UPBdata(j,1,i)/4.3); %204
		end
	end
end
UPBdata(44:46,2,:) = [UPBdata(43,2,:);UPBdata(43,2,:);UPBdata(43,2,:)];

deadtime = str2num(get(H.dt,'String')); %slider for change in deadtime

for i = 1:data_count
	for j = 1:46
		if values_all(j+10,3,i) - UPB_pre(i,3) < 1
			UPBdata(j,3,i) = 1;
		else
			UPBdata(j,3,i) = (values_all(j+10,3,i) - UPB_pre(i,3)) / (1-(values_all(j+10,3,i)-UPB_pre(i,3))*deadtime/1000000000); %206
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if values_all(j+10,4,i) - UPB_pre(i,4) < 1
			UPBdata(j,4,i) = 1;
		else
			UPBdata(j,4,i) = (values_all(j+10,4,i)-UPB_pre(i,4))*(1+lowint67*exp(-1*(values_all(j+10,4,i)-UPB_pre(i,4))/10000) + lin67*(values_all(j+16,4,i)-UPB_pre(i,4))/10000); % 207
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if values_all(j+10,5,i) - UPB_pre(i,5) < 1
			UPBdata(j,5,i) = 1;
		else
			UPBdata(j,5,i) = (values_all(j+10,5,i)-UPB_pre(i,5))/(1-(values_all(j+10,5,i)-UPB_pre(i,5))*deadtime/1000000000); %208
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if values_all(j+10,6,i) - UPB_pre(i,6) < 1
			UPBdata(j,6,i) = 1;
		else
			UPBdata(j,6,i) = (values_all(j+10,6,i)-UPB_pre(i,6))/(1-(values_all(j+10,6,i)-UPB_pre(i,6))*deadtime/1000000000); %232
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if values_all(j+10,7,i) - UPB_pre(i,7) < 1
			UPBdata(j,7,i) = 1;
		else
			UPBdata(j,7,i) = (values_all(j+10,7,i)-UPB_pre(i,7)) / (1-(values_all(j+10,7,i)-UPB_pre(i,7))*deadtime/1000000000); %235
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if UPBdata(j,7,i)*137.82 > 5000000
			...
			UPBdata(j,8,i) = UPBdata(j,7,i)*(1+0.2*lowint68*exp(-0.000001*(UPBdata(j,7,i)*137.82))); %235*
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if values_all(j+10,8,i) - UPB_pre(i,8) < 1
			UPBdata(j,9,i) = 1;
		else
			UPBdata(j,9,i) = (values_all(j+10,8,i)-UPB_pre(i,8))/(1-(values_all(j+10,8,i)-UPB_pre(i,8))*deadtime/1000000000); %238
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if UPBdata(j,9,i) > 5000000
			...
			UPBdata(j,10,i) = UPBdata(j,9,i)*(1+0.2*lowint68*exp(-0.000001*UPBdata(j,9,i))); %238*
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if UPBdata(j,3,i) == 1 || UPBdata(j,10,i) == 1
			UPBdata(j,11,i) = 1;
		else
			UPBdata(j,11,i) = UPBdata(j,3,i)/UPBdata(j,10,i); %206/238
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if UPBdata(j,3,i) == 1 || UPBdata(j,4,i) == 1
			UPBdata(j,12,i) = 1;
		elseif UPBdata(j,3,i)/UPBdata(j,4,i) > 30
			UPBdata(j,12,i) = 30;
		elseif UPBdata(j,3,i)/UPBdata(j,4,i) < 1.5
			UPBdata(j,12,i) = 1.5;
		else
			UPBdata(j,12,i) = UPBdata(j,3,i)/UPBdata(j,4,i); %206/207
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		...
		elseif abs(UPBdata(j,3,i)/UPBdata(j,2,i)) < 100
			...
			UPBdata(j,13,i) = abs(UPBdata(j,3,i)/UPBdata(j,2,i)); %206/204
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if UPBdata(j,5,i) == 1 || UPBdata(j,6,i) == 1
			UPBdata(j,14,i) = 1;
		else
			UPBdata(j,14,i) = UPBdata(j,5,i)/UPBdata(j,6,i); %208/232
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if UPBdata(j,5,i) == 1
			UPBdata(j,15,i) = 1;
		elseif abs(UPBdata(j,5,i)/UPBdata(j,2,i)) > 10000
...
for i = 1:data_count
	if mean(UPBdata(2:33,10,i)) < 50000 || mean(UPBdata(2:33,2,i)) > 100000
		mode(i,1) = {'bad'};
	elseif countsum(1,i) < 3
		mode(i,1) = {'IC'};
	elseif mean(UPBdata(2:33,10,i)) > 5000000
		mode(i,1) = {'AN'};
	else
		mode(i,1) = {'MI'};
	end
...
%%%%%%%%%%%%% ORIGINAL %%%%%%%%%%%%%%%
for i = 1:data_count
	tbl = table((1:1:31)',UPBdata(3:33,11,i));
	mdl = fitlm(tbl);
...
for i = 1:data_count
	tbl = table((1:1:25)',UPBdata(4:28,14,i));
	mdl = fitlm(tbl);
...
for i = 1:data_count
	UPB_reduced(i,1) = abs(mean(UPBdata(3:33,2,i)));
	UPB_reduced(i,2) = abs(mean(UPBdata(3:33,3,i)));
	UPB_reduced(i,3) = abs(mean(UPBdata(3:33,4,i)));
	UPB_reduced(i,4) = abs(mean(UPBdata(3:33,5,i)));
	if mean(UPBdata(3:33,6,i)) < 1000
		UPB_reduced(i,5) = 1;
	else
		UPB_reduced(i,5) = abs(mean(UPBdata(3:33,6,i)));
	end
	if mean(UPBdata(3:33,8,i)) < 1000
		UPB_reduced(i,6) = 1;
	else
		UPB_reduced(i,6) = abs(mean(UPBdata(3:33,8,i)));
	end
	if mean(UPBdata(3:33,10,i)) < 1000
		UPB_reduced(i,7) = 1;
	else
		UPB_reduced(i,7) = abs(mean(UPBdata(3:33,10,i)));
	end
end
...
for i = 1:data_count
	if strcmp(mode{i,1}, 'bad') == 1
		UPB_reduced(i,8) = 1.3;
	elseif use_235 == 1 && strcmp(mode{i,1}, 'IC') == 0
...
for i = 1:data_count
	...
	elseif 100*fit68_err(i,1)/UPB_reduced(i,8) > 50
		...
		UPB_reduced(i,9) = 0.6*100*fit68_err(i,1)/UPB_reduced(i,8); %underdispersion factor applied of 0.6
	end
end
...
for i = 1:data_count
	if strcmp(mode{i,1}, 'bad') == 1
		UPB_reduced(i,12) = 1;
	elseif 100*std(UPBdata(3:33,12,i))/UPB_reduced(i,11)/sqrt(31) > 50
		UPB_reduced(i,12) = 50;
	else
		UPB_reduced(i,12) = 0.6*100*std(UPBdata(3:33,12,i))/UPB_reduced(i,11)/sqrt(31); %underdispersion factor applied of 0.6
	end
end
...
for i = 1:data_count
	if strcmp(mode{i,1}, 'bad') == 1
		UPB_reduced(i,14) = 1;
	elseif (100*std(UPBdata(3:33,13,i))/UPB_reduced(i,13))/sqrt(31) > 100
		UPB_reduced(i,14) = 100;
	else
		UPB_reduced(i,14) = (100*std(UPBdata(3:33,13,i))/UPB_reduced(i,13))/sqrt(33);
	end
end
...
for i = 1:data_count
	if strcmp(mode{i,1}, 'bad') == 1
		UPB_reduced(i,18) = 1;
	elseif 100*std(UPBdata(3:33,15,i))/UPB_reduced(i,17)/sqrt(31) > 50
		UPB_reduced(i,18) = 50;
	else
		UPB_reduced(i,18) = 100*std(UPBdata(3:33,15,i))/UPB_reduced(i,17)/sqrt(31);
	end
end
...
%=DT4*(1+0.1*MASTER2!$AH$13*EXP(-0.000002*J4))
...
CORRECTED_CONC_RATIOS(2:end,:) = [sample, num2cell(Uppm), num2cell(Thppm), num2cell(CY), num2cell(UPB_reduced(:,17)), ...
	num2cell(UTh), num2cell(fcbc67), num2cell(re67), num2cell(fudge82), num2cell(re82), num2cell(ratio75), num2cell(ratio75err), num2cell(ratio68), ...
...
if TREE == 1
	H.STD_NISTpm = STD_NISTpm;
	H.STD_NISTps = STD_NISTps;
	H.STD_SLpm = STD_SLpm;
	H.STD_SLps = STD_SLps;
	H.STD_FCpm = STD_FCpm;
	H.STD_FCps = STD_FCps;
	...
	H.SL = SL;
	H.Isotopes = Isotopes;
	H.ChonNormSL = ChonNormSL;
	H.ChonNormFC = ChonNormFC;
	H.ChonNormNIST = ChonNormNIST;
	H.ChonNormAcceptedNIST = ChonNormAcceptedNIST;
	H.ChonNormAcceptedSL = ChonNormAcceptedSL;
	H.ChonNormAcceptedFC = ChonNormAcceptedFC;
...
for i = 1:data_count
	if cell2num(Macro_1_2_Output(i+1,13)) < 10
		syst_err_68(i,1) = sqrt(100*ffswse68(i,1)/ffsw68(i,1)*100*ffswse68(i,1)/ffsw68(i,1)+pbcerr68(i,1)*pbcerr68(i,1)+0.053*0.053+0.2*0.2);
	else
		syst_err_68(i,1) = 0;
	end
end
...
for i = 1:data_count
	if cell2num(Macro_1_2_Output(i,16)) < 10 && Age68(i,1) > str2num(get(H.filter_cutoff,'String'))
		syst_err_67(i,1) = sqrt(100*ffswse67(i,1)/ffsw67(i,1)*100*ffswse67(i,1)/ffsw67(i,1)+(pbcerr67(i,1))*(pbcerr67(i,1))+0.053*0.053+0.069*0.069+0.2*0.2);
	end
end
...
if TREE == 1 && get(H.tree,'Value') == 1
	...
	set(H.tSL,'Visible','on')
	set(H.tFC,'Visible','on')
	set(H.sliderSL,'Visible','on')
	set(H.sliderFC,'Visible','on')
	set(H.calibslider,'Visible','on')
	%set(H.treeplotter,'Visible','on')
	
	STD_NISTpm = H.STD_NISTpm;
	STD_NISTps = H.STD_NISTps;
	STD_SLpm = H.STD_SLpm;
	STD_SLps = H.STD_SLps;
	STD_FCpm = H.STD_FCpm;
	STD_FCps = H.STD_FCps;
	...
	SL = H.SL;
	Isotopes = H.Isotopes;
	ChonNormSL = H.ChonNormSL;
	ChonNormFC = H.ChonNormFC;
	ChonNormNIST = H.ChonNormNIST;
	ChonNormAcceptedNIST = H.ChonNormAcceptedNIST;
	ChonNormAcceptedSL = H.ChonNormAcceptedSL;
	ChonNormAcceptedFC = H.ChonNormAcceptedFC;
	
	
	axes(H.TREEcalib)
	hold on
	plot([(1:1:25); (1:1:25)], [(STD_NISTpm+STD_NISTps); (STD_NISTpm-STD_NISTps)], '-r', 'Color', 'g', 'LineWidth',4) % Error bars
	plot([(1:1:25); (1:1:25)], [(STD_SLpm+STD_SLps); (STD_SLpm-STD_SLps)], '-r', 'Color', 'r', 'LineWidth',4) % Error bars
	plot([(1:1:25); (1:1:25)], [(STD_FCpm+STD_FCps); (STD_FCpm-STD_FCps)], '-r', 'Color', 'b', 'LineWidth',4) % Error bars
	plot([1:1:25],zeros(1,25),'k','LineWidth',2)
	plot([1:1:25],STD_NISTpm,'g','LineWidth',2)
	plot([1:1:25],STD_SLpm,'r','LineWidth',2)
	plot([1:1:25],STD_FCpm,'b','LineWidth',2)
	s1 = scatter([1:1:25], STD_NISTpm, 50, 'g', 'filled','s','MarkerEdgeColor','k');
	s2 = scatter([1:1:25], STD_SLpm, 50, 'r', 'filled','d','MarkerEdgeColor','k');
	s3 = scatter([1:1:25], STD_FCpm, 50, 'b', 'filled','o','MarkerEdgeColor','k');
	legend([s1 s3 s2], [{NIST}, {FC}, {SL}], 'Location','northeast');
	xlim ([0.5 25.5]);
	ax1 = gca;
	ax1.XTick = [1:1:25];
	...
	axes(H.TREEnorm)
	hold on
	for i = 1:length(ChonNormSL(:,1))
		plot([1:1:14],ChonNormSL(i,8:21),'--', 'Color', [.6 0 0], 'LineWidth',1)
	end
	for i = 1:length(ChonNormFC(:,1))
		plot([1:1:14],ChonNormFC(i,8:21),'--', 'Color', [0 0 .6],'LineWidth',1)
	end
	for i = 1:length(ChonNormNIST(:,1))
		plot([1:1:14],ChonNormNIST(i,8:21),'--', 'Color', [0 .6 0], 'LineWidth',1)
	end
	plot([1:1:14],ChonNormAcceptedNIST,'g','LineWidth',2)
	plot([1:1:14],ChonNormAcceptedSL,'r','LineWidth',2)
	plot([1:1:14],ChonNormAcceptedFC,'b','LineWidth',2)
	s4 = scatter([1:1:14], ChonNormAcceptedNIST, 50, 'g', 'filled','s','MarkerEdgeColor','k');
	s5 = scatter([1:1:14], ChonNormAcceptedSL, 50, 'r', 'filled','d','MarkerEdgeColor','k');
	s6 = scatter([1:1:14], ChonNormAcceptedFC, 50, 'b', 'filled','o','MarkerEdgeColor','k');
	legend([s4 s6 s5], [{'NIST Accepted'}, {'FC Accepted'}, {'SL Accepted'}], 'Location','southeast');
	xlim ([0.5 14.5]);
	ax2 = gca;
	ax2.XTick = [1:1:14];
...
function sliderSL_Callback(hObject, eventdata, H)
perc_SL = str2num(get(H.sliderSL,'String'))*.01;
perc_FC = 1 - perc_SL;
set(H.calibslider,'Value',1 - perc_SL)
set(H.sliderFC,'String',round(perc_FC*100,1))
guidata(hObject, H);

function sliderFC_Callback(hObject, eventdata, H)
perc_FC = str2num(get(H.sliderFC,'String'))*.01;
perc_SL = 1 - perc_FC;
set(H.calibslider,'Value',perc_FC)
set(H.sliderSL,'String',(1 - perc_FC)*100)
guidata(hObject, H);
...
for i = 1:data_count
	for j = 1:46
		if values_all(j+10,4,i) - UPB_pre(i,4) < 1
			...
			UPBdata2(j,4,i) = (values_all(j+10,4,i)-UPB_pre(i,4))*(1+lowint67*exp(-1*(values_all(j+10,4,i)-UPB_pre(i,4))/10000) + lin67*(values_all(j+16,4,i)-UPB_pre(i,4))/10000); % 207
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if values_all(j+10,7,i) - UPB_pre(i,7) < 1
			...
			UPBdata2(j,7,i) = (values_all(j+10,7,i)-UPB_pre(i,7)) / (1-(values_all(j+10,7,i)-UPB_pre(i,7))*deadtime/1000000000); %235
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if UPBdata2(j,7,i)*137.82 > 5000000
			...
			UPBdata2(j,8,i) = UPBdata2(j,7,i)*(1+0.2*lowint68*exp(-0.000001*(UPBdata2(j,7,i)*137.82))); %235*
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if UPBdata2(j,9,i) > 5000000
			...
			UPBdata2(j,10,i) = UPBdata2(j,9,i)*(1+0.2*lowint68*exp(-0.000001*UPBdata2(j,9,i))); %238*
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		...
		UPBdata2(j,4,i) = (values_all(j+10,4,i)-UPB_pre(i,4))*(1+lowint67*exp(-1*(values_all(j+10,4,i)-UPB_pre(i,4))/10000) + lin67*(values_all(j+10,4,i)-UPB_pre(i,4))/10000);
...
for i = 1:data_count
	if mean(UPBdata2(2:33,10,i)) < 50000 || mean(UPBdata2(2:33,2,i)) > 100000
		mode(i,1) = {'bad'};
	elseif countsum(1,i) < 3
		mode(i,1) = {'IC'};
	elseif mean(UPBdata2(2:33,10,i)) > 5000000
		mode(i,1) = {'AN'};
	else
		mode(i,1) = {'MI'};
	end
...
for i = 1:data_count
	for j = 1:46
		if UPBdata2(j,3,i) == 1 || UPBdata2(j,10,i) == 1
			UPBdata2(j,11,i) = 1;
		else
			UPBdata2(j,11,i) = UPBdata2(j,3,i)/UPBdata2(j,10,i); %206/238
		end
	end
end
...
for i = 1:data_count
	for j = 1:46
		if UPBdata2(j,3,i) == 1 || UPBdata2(j,4,i) == 1
			UPBdata2(j,12,i) = 1;
		elseif UPBdata2(j,3,i)/UPBdata2(j,4,i) > 30
			UPBdata2(j,12,i) = 30;
		elseif UPBdata2(j,3,i)/UPBdata2(j,4,i) < 1.5
			UPBdata2(j,12,i) = 1.5;
		else
			UPBdata2(j,12,i) = UPBdata2(j,3,i)/UPBdata2(j,4,i); %206/207
		end
	end
end
...
UPB_reduced = zeros(data_count,18);
for i = 1:data_count
	UPB_reduced(i,1) = abs(mean(UPBdata2(3:33,2,i)));
	UPB_reduced(i,2) = abs(mean(UPBdata2(3:33,3,i)));
	UPB_reduced(i,3) = abs(mean(UPBdata2(3:33,4,i)));
	UPB_reduced(i,4) = abs(mean(UPBdata2(3:33,5,i)));
	if mean(UPBdata2(3:33,6,i)) < 1000
		UPB_reduced(i,5) = 1;
	else
		UPB_reduced(i,5) = abs(mean(UPBdata2(3:33,6,i)));
	end
	if mean(UPBdata2(3:33,8,i)) < 1000
		UPB_reduced(i,6) = 1;
	else
		UPB_reduced(i,6) = abs(mean(UPBdata2(3:33,8,i)));
	end
	if mean(UPBdata2(3:33,10,i)) < 1000
		UPB_reduced(i,7) = 1;
	else
		UPB_reduced(i,7) = abs(mean(UPBdata2(3:33,10,i)));
	end
end
...
for i = 1:data_count
	if strcmp(mode{i,1}, 'bad') == 1
		UPB_reduced(i,12) = 1;
	elseif 100*std(UPBdata2(3:33,12,i))/UPB_reduced(i,11)/sqrt(31) > 50
		UPB_reduced(i,12) = 50;
	else
		UPB_reduced(i,12) = 0.6*100*std(UPBdata2(3:33,12,i))/UPB_reduced(i,11)/sqrt(31); %underdispersion factor applied of 0.6
	end
end
...
for i = 1:data_count
	if strcmp(mode{i,1}, 'bad') == 1
		UPB_reduced(i,14) = 1;
	elseif (100*std(UPBdata2(3:33,13,i))/UPB_reduced(i,13))/sqrt(31) > 100
		UPB_reduced(i,14) = 100;
	else
		UPB_reduced(i,14) = (100*std(UPBdata2(3:33,13,i))/UPB_reduced(i,13))/sqrt(33);
	end
end
...
for i = 1:data_count
	if strcmp(mode{i,1}, 'bad') == 1
		UPB_reduced(i,18) = 1;
	elseif 100*std(UPBdata2(3:33,15,i))/UPB_reduced(i,17)/sqrt(31) > 50
		UPB_reduced(i,18) = 50;
	else
		UPB_reduced(i,18) = 100*std(UPBdata2(3:33,15,i))/UPB_reduced(i,17)/sqrt(31);
	end
end
...
function filtAl_Callback(hObject, eventdata, handles)
% hObject    handle to filtAl (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of filtAl as text
%        str2double(get(hObject,'String')) returns contents of filtAl as a double


% --- Executes during object creation, after setting all properties.
function filtAl_CreateFcn(hObject, eventdata, handles)
% hObject    handle to filtAl (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
function filtP_Callback(hObject, eventdata, handles)
% hObject    handle to filtP (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of filtP as text
%        str2double(get(hObject,'String')) returns contents of filtP as a double


% --- Executes during object creation, after setting all properties.
function filtP_CreateFcn(hObject, eventdata, handles)
% hObject    handle to filtP (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
